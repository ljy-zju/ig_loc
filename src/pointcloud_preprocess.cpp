...
void PointCloudPreprocess::ProcessOuster(
  ...
  for (size_t i = 0; i < cloud_origin.size(); ++i) {
    if ((i % config_.point_filter_num == 0) && !HasInf(cloud_origin.at(i)) &&
      ...
      point.curvature = (i % 2048) / 20480.0 * 1e3f; // fp
      cloud_out->push_back(point);
    }
  }
}
...
void PointCloudPreprocess::ProcessHesai(
  ...
  double time_start = msg->header.stamp.toSec(); // s
  ...
  for (size_t i = 0; i < cloud_origin.size(); ++i) {
    if ((i % config_.point_filter_num == 0) && !HasInf(cloud_origin.at(i)) &&
      ...
      // ms
      point.curvature =
          (cloud_origin.at(i).timestamp - time_start) * config_.time_scale;
      // LOG(INFO) << "time one pt = " <<
      // std::to_string(cloud_origin.at(i).timestamp); LOG(INFO) << "float time
      // one pt = " << std::to_string(point.curvature);

      cloud_out->push_back(point);
    }
  }
  // LOG(INFO) << "time diff in one frame = " <<
  // std::to_string(cloud_out->begin()->curvature -
  // (cloud_out->end()-1)->curvature);
}

template <typename T> inline bool PointCloudPreprocess::HasInf(const T &p) {
  return (std::isinf(p.x) || std::isinf(p.y) || std::isinf(p.z));
}

template <typename T> inline bool PointCloudPreprocess::HasNan(const T &p) {
