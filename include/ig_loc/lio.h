...
#include "ig_loc/faster_voxel_grid.h"
#include "ig_loc/point_type.h"
#include "ig_loc/utilities.hpp"
...
public:
  struct Config {
    ...
    bool init_ok{false};
    bool en_map_update{false};
  };

  LIO(Config config = Config())
      : config_(config), cloud_cov_ptr_(new CloudCovType()),
  ...
  void SetPriorMap(pcl::PointCloud<PointType>::Ptr prior_map_ptr);

  void SetInitPose(const Eigen::Matrix4d &init_pose) {
    init_pose_map2world_ = init_pose;
  }
...
private:
  ...
  Eigen::Matrix4d init_pose_map2world_ = Eigen::Matrix4d::Identity();
  State curr_state_;
  State prev_state_;
